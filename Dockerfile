...
# Stage 2: Build Sierra-HBV WAR (postalign + minimap2 pulled from base image for tests)
...
RUN mv build/libs/Sierra-HBV-*.war build/libs/Sierra-HBV.war 2>/dev/null

# Stage 3: Runtime image
FROM hivdb/tomcat-with-postalign:latest
ENV CATALINA_OPTS="-Xms1024M -Xmx6144M -XX:+UseG1GC -XX:MaxHeapFreeRatio=30 -XX:MinHeapFreeRatio=10"
COPY --from=builder /sierra/build/libs/Sierra-HBV.war /usr/share/tomcat/webapps
